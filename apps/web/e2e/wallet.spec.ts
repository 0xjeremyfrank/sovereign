...
test.describe('Wallet Connection UI', () => {
  test('shows connect wallet button when not connected', async ({ page }) => {
    ...
    const connectButton = page.getByRole('button', { name: /connect/i }).or(
      page.locator('[data-testid="connect-wallet"]')
    );
  ...
  test('admin page shows connect prompt when not connected', async ({ page }) => {
    ...
    await expect(page.getByText(/connect.*wallet.*admin/i).or(
      page.getByText(/connect wallet to access/i)
    )).toBeVisible();
  });
  ...
  test('connect button opens wallet modal', async ({ page }) => {
    ...
    if (hasModal) {
      ...
      await expect(walletOption.first()).toBeVisible({ timeout: 5000 }).catch(() => {
