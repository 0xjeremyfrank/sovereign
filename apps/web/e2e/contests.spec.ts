...
test.describe('Contest Browsing', () => {
  ...
  test('shows loading or content state', async ({ page }) => {
    ...
    const skeletons = page.locator('[class*="skeleton"]').or(page.locator('[class*="animate-pulse"]'));
    const content = page.getByText(/no contests found/i).or(page.locator('[data-testid="contest-card"]'));
  ...
  test('displays page content or loading state', async ({ page }) => {
    ...
    await expect(skeletons.first().or(emptyState).or(contestCards.first())).toBeVisible({ timeout: 10000 });
  });

  test('can navigate back to sandbox from contests', async ({ page }) => {
    // Find navigation link to home/sandbox
    const homeLink = page.getByRole('link', { name: /sandbox|play|home/i }).or(
      page.locator('a[href="/"]')
    );
